...
- [x] 明确首页主任务路径：确认入库、选择知识点生成题目、答题评分、查看掌握画像。
- [x] 首页新增“下一步建议”，按当前数据状态提示当前可执行步骤。
- [x] 首页新增最近对象快捷操作：待确认编辑、Agent 沉淀、最近知识点生成题目、题目答题、练习安排。
- [x] 将运行时状态更新为 MCP stdio 可调用，避免继续显示“预留”。
- [x] 补充首页 E2E 断言，覆盖任务导向工作台的首屏信号。
- [x] 执行 TypeScript 类型检查。
- [x] 执行单元和集成测试。
- [x] 执行 Next.js 生产构建。
- [x] 执行文档时间戳检查。
...
```text
2026-06-11 10:37:23 第 51 章启动：在第 50 章完成且无未完成任务后，新增 Web 首屏任务导向工作台；目标是解决首页不容易判断“如何选择知识点和生成题目”的体验问题。
2026-06-11 10:44:16 首页工作台优化：新增“下一步建议”阶段条和最近对象快捷操作，覆盖待确认编辑、Agent 沉淀、最近知识点生成题目、题目答题和练习安排；运行时状态更新为 MCP stdio 可调用。
2026-06-11 10:44:16 首页 E2E 更新：补充“下一步建议”“选择知识点生成题目”和“MCP stdio 可调用”首屏断言。
2026-06-11 10:44:16 cmd /c npx tsc --noEmit：通过。
2026-06-11 10:44:16 cmd /c npm run test：通过，23 个测试文件，68 条测试用例。
2026-06-11 10:44:16 cmd /c npm run build：通过，Web 首屏任务导向工作台进入 Next.js 生产构建。
